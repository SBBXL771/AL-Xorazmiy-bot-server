...
const ADMIN_CHAT_ID2 = parseInt(process.env.ADMIN_CHAT_ID2) || 0;
const ADMIN_PASSWORD2 = process.env.ADMIN_PASSWORD2 || "admin123";
let startups;
try {
...
function isAdmin(chatId) {
    return chatId === ADMIN_CHAT_ID || chatId === ADMIN_CHAT_ID1 || chatId === ADMIN_CHAT_ID2;
}
...
bot.on('message', (msg) => {
    ...
    if (session?.mode === "awaiting_password") {
        if (text === ADMIN_PASSWORD || text === ADMIN_PASSWORD1 || text === ADMIN_PASSWORD2) {
        ...
        if (!isNaN(Date.parse(date))) {
            ...
            bot.sendMessage(chatId, `💰 ${startupIds.map(id => startups[id].name).join(', ')} uchun ${text} sanasi uchun aksiya narxini kiriting (masalan: '1:1.2 or,3:15'):`).catch(err => sendErrorMessage(chatId, err));
        } else {
